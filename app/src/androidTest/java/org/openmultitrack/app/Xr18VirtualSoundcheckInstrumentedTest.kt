...
/**
 * XR18 is not physically connected yet — validates 18-channel playback descriptor parsing
 * for future virtual soundcheck (multichannel playback + multichannel capture loop).
 */
...
class Xr18VirtualSoundcheckInstrumentedTest {
    ...
    fun syntheticDescriptorSupportsEighteenChannelVirtualSoundcheck() {
        ...
        val capture = NativeUac2Probe.selectBestCaptureAlt(caps, minChannels = 18)!!

        assertThat(playback.channels).isEqualTo(18)
        assertThat(capture.channels).isEqualTo(18)
        assertThat(playback.sampleRateHz).isEqualTo(48_000)
        ...
        fun addAsGeneral(terminal: Int, channels: Int) {
            add(
                byteArrayOf(
                    ...
                    0x01, 0x00, 0x00, 0x00, channels.toByte(), 0x00, 0x00, 0x00, 0x00, 0x00,
                ),
            )
        }
        ...
        fun addEp(addr: Int, maxPacket: Int) {
            add(
                byteArrayOf(
                    ...
                    (maxPacket and 0xFF).toByte(), ((maxPacket shr 8) and 0xFF).toByte(), 0x01,
                ),
            )
        }
        addIface(1, 1, 1, 0x20)
